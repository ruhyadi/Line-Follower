...
class LineFollowerEnv(gym.Env):
    ...
    def __init__(self, gui=True, nb_cam_pts=8, sub_steps=10, sim_time_step=1 / 250,
        ...
        if self.obsv_type == "points_visible":
            ...
            self.observation_space = spaces.Box(low=0, high=255, shape=(100, 240, 3), dtype=np.uint8)
...
if __name__ == '__main__':
    ...
    env = LineFollowerEnv(gui=True, nb_cam_pts=8, max_track_err=0.4, power_limit=0.4, max_time=600, obsv_type="camera")
    env.reset()
    for _ in range(100):
        for i in range(1000):
